...
func init() {
	android.RegisterModuleType("hexa_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = hexaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = hexaExpandVariables(ctx, dep_file)
	...
	cmd := hexaExpandVariables(ctx, String(g.properties.Cmd))
